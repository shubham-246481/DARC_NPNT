...
#Function to check mission parameters
def mission_check(polygon):
  ...
  for i in range(len(cmds)):
    ...
    pts = []
    pts.append((lat,lon))
    g = pts.within(polygon)
    if not g:
      print("GEOFENCE VIOLATED AT WAYPOINT"+str(i+1))
      return 0
    else:
      print("ALL POINTS WITHIN GEOFENCE")
      return 1
...
flightParams, geoFence = xml_parses(afile)
...
if time_flag:
  mission = mission_check(polygon=polygon)
  if mission:
